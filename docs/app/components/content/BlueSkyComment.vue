...
<template>
  <div
    class="border-b border-primary/10 dark:border-primary/50 pt-4 last:border-b-0"
  ...
  >
    <section class="hover:opacity-60 cursor-pointer transition-all">
      <div class="flex items-center gap-3 px-4">
        <UiAvatar>
          <UiAvatarImage
            v-if="thread.post.author.avatar"
            :src="getAvatarUrl(thread.post.author.avatar)"
        ...
        <NuxtLink
          :href="getAuthorProfileUrl(thread.post.author.did)"
          class="grid gap-1"
        >
        ...
        <NuxtLink
          :href="getPostUrl(thread.post.author.did, thread.post.uri)"
          target="_blank"
          rel="ugc"
